...
/**
 * ACLED's Terms & Conditions §1.2: "Commercial entities may not access or
 * use the Content and/or Platforms without first obtaining a corporate
 * license" (acleddata.com/terms-of-use). Bruce ratified serving this anyway
 * while small, 2026-08-18 — NOT zero-rated (billed like any other pack; see
 * the gateway's `acled` pack entry). Attribution Policy
 * (acleddata.com/attributionpolicy): "ACLED as the source of the data must
 * be clearly and prominently acknowledged. This applies to uses of the
 * original ACLED data as well as any manipulation of or additions to the
 * data." Fixed here via the shared LicenseTerms model (fleet #1976).
 */
const ACLED_LICENSE = vendorTermsLicense({
  ref: 'ACLED-Attribution-Policy',
  url: 'https://acleddata.com/attributionpolicy/',
  attribution: 'Data from the Armed Conflict Location & Event Data Project (ACLED). https://acleddata.com/',
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Acled');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  return attachLicense(await dispatch(name, args), ACLED_LICENSE);
}
...
function credentials(args: Record<string, unknown>): { email: string; password: string } {
  ...
  if (!email || !password) {
    // "account-only" is load-bearing: it is what routes this to auth_required in
    // the gateway's classifyToolError. Before the reword this guard classed as
    // upstream_down, which reads as "wait for ACLED to recover" — nobody chases
    // a credential on that signal.
    throw new Error(
      'ACLED is account-only (BYO). Pass ?_email=...&_password=... on the gateway URL with your myACLED credentials. Register at https://acleddata.com/register/ and then email access@acleddata.com to request API activation (registration alone is not enough — every account needs manual approval).',
    );
  }
...
async function acledFetch(args: Record<string, unknown>, extraLimit?: number): Promise<EventRow[]> {
  ...
  let res = await pwFetch(`${READ_URL}?${params}`, {
  ...
  if (res.status === 401) {
    ...
    res = await pwFetch(`${READ_URL}?${params}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
    });
  }
